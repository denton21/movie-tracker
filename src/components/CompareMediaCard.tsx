...
interface CompareMediaCardProps {
    media: Media;
    userMedia?: UserMedia | null;
    friendMedia?: UserMedia | null;
}

export default function CompareMediaCard({
    media,
    userMedia,
    friendMedia,
}: CompareMediaCardProps) {
    ...
    return (
        <>
            <div className="group relative glass rounded-2xl overflow-hidden card-hover flex flex-col h-full">
                {/* Постер - клик открывает страницу медиа */}
                <Link href={`/media/${media.media_type}-${media.tmdb_id}`} className="block relative aspect-[2/3] overflow-hidden bg-zinc-900/50">
                    {media.poster_path ? (
                        <Image
                    ...
                    {/* Статус пользователя (оригинальный) */}
                    {statusConfig && (
                        <div className={`absolute bottom-[4.5rem] left-4 right-4 px-3 py-2 rounded-xl backdrop-blur-md border shadow-lg ${status === 'watching' ? 'bg-blue-500/10 border-blue-500/20 text-blue-300' :
                ...
                {media.media_type === 'tv' && userMedia && userMedia.status !== 'planned' && media.total_episodes && (
                    <div className="px-4 pb-2 pt-1 bg-gradient-to-b from-transparent to-black/20">
                        <ProgressBar
                ...
                {/* Сравнение статусов */}
                <div className="mt-auto p-4 bg-zinc-900/50 border-t border-white/5 grid grid-cols-2 gap-3 relative z-10 backdrop-blur-xl">
                    {/* Мой статус - кликабельный */}
                    <button
                        onClick={() => setIsEditorOpen(true)}
                        className="text-center p-3 rounded-xl hover:bg-white/10 transition-colors group/btn border border-transparent hover:border-white/10 flex flex-col justify-between h-full"
                    >
                        <p className="text-slate-400 text-xs font-semibold tracking-wider uppercase mb-1.5">Вы</p>
                        <UserStatusBadge userMedia={userMedia} />
                        <p className="text-rose-400 text-xs mt-2 opacity-0 group-hover/btn:opacity-100 transition-opacity font-medium">
                            ✏️ Изменить
                        </p>
                    </button>

                    {/* Статус друга - только отображение */}
                    <div className="text-center p-3 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-between h-full">
                        <p className="text-slate-400 text-xs font-semibold tracking-wider uppercase mb-1.5">Друг</p>
                        <UserStatusBadge userMedia={friendMedia} />
                        <div className="h-4 mt-2"></div> {/* Placeholder для выравнивания */}
                    </div>
                </div>
            </div>
...
function UserStatusBadge({ userMedia }: { userMedia?: UserMedia | null }) {
    const status = userMedia?.status as WatchStatus | undefined;
